...
- A Run fetches at most eight Feeds concurrently. Each Feed request has a bounded transport retry policy: at most three attempts (initial request plus retries after 250 ms and 1 s) for timeouts, connection failures, and HTTP 408/429/5xx responses. Parse failures and other non-transient responses are not retried. Existing per-Feed `record_failure`, exponential backoff, `next_fetch`, failure count, and automatic-disable rules remain the only durable retry policy; transport retries do not increment the Feed failure count until the request ultimately fails.
